...
if ( ! defined('ABSPATH')) exit;  // if direct access
	...
	foreach($layout as $item_id=>$item_info){
		...
		$item['author_link'] = '<a class="element element_'.$item_id.' '.$item_key.'" href="'.get_author_posts_url(get_the_author_meta( 'ID' ), get_the_author_meta( 'user_nicename' )).'">'.get_the_author().'</a>';		
				...
				if ( ! empty( $categories ) ) {
					foreach( $categories as $category ) {
						$html_categories .= '<a class="'.$category->slug.' category-'.$category->term_id.'" href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( __( 'View all posts in %s', post_grid_textdomain ), $category->name ) ) . '">' . esc_html( $category->name ) . '</a>' . $separator;
		...
		if(in_array( 'yith-woocommerce-wishlist/init.php', (array) $active_plugins ) || in_array( 'yith-woocommerce-wishlist-premium/init.php', (array) $active_plugins )){
		...
		if(in_array( 'woocommerce/woocommerce.php', (array) $active_plugins ) && $is_product=='product'){
			...
			$wc_gallery_html = '';
			$gallery_attachment_ids = array_filter($product->get_gallery_image_ids());
			...
			if(in_array( 'woocommerce/woocommerce.php', (array) $active_plugins ) && $is_product=='product'){
				...
				if(in_array( 'woocommerce/woocommerce.php', (array) $active_plugins ) && $is_product=='product'){
					...
					if( $rating > 0 ){
						...
						$wc_rating_text_html = ' '.sprintf(__('%s out of 5', post_grid_textdomain), $rating);
			...
			if(in_array( 'woocommerce/woocommerce.php', (array) $active_plugins ) && $is_product=='product'){
					...
					$categories = wc_get_product_category_list($item_post_id);
				...
				if(in_array( 'woocommerce/woocommerce.php', (array) $active_plugins ) && $is_product=='product'){
					...
					$tags = wc_get_product_tag_list($item_post_id);

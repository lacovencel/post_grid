...
class class_post_grid_license{
	...
	function days_remaining($date_expiry) {
		...
		if($diff<0){
			...
			$response['diff'] = $diff;			
			$response['text'] = __('Expired', post_grid_textdomain);
			...
			if($year>0){
				$response['text'] =  number_format_i18n($year) .' '.__('year', post_grid_textdomain);
				}
					
			elseif($month > 0 && $day<=12 ){
				$response['text'] = number_format_i18n($month) .' '.__('month', post_grid_textdomain);
				}
				
			elseif($day > 0 && $day<=30){
				$response['text'] = number_format_i18n($day).' '.__('day', post_grid_textdomain);
				}
				
			elseif($hour > 0 && $hour<=24){
				$response['text'] = number_format_i18n($hour).' '.__('hour', post_grid_textdomain);
				}		
				
			elseif($minute > 0 && $minute<60){
				$response['text'] = number_format_i18n($minute).' '.__('minute', post_grid_textdomain);
				}	
					
			else{
				$response['text'] = $diff.' '.__('second', post_grid_textdomain);;
				}
	...
	// var_dump($license_status);
		...
		if(empty($license_status)){
				...
				$html.= sprintf(__('Please activate your license for <b>%s &raquo; <a href="%sedit.php?post_type=post_grid&page=license">License</a></b>', post_grid_textdomain), post_grid_plugin_name, $admin_url);

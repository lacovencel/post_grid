...
function post_grid_add_shortcode_column( $columns ) {
    return array_merge( $columns, 
        array( 'shortcode' => __( 'Shortcode', post_grid_textdomain ) ) );
}
...
add_filter('post_grid_filter_skins','post_grid_filter_skins');

function post_grid_filter_skins($skins){

  $skins['new_skin'] = array(
    'slug'=>'new-skin',
    'name'=>__('SC-Custom','post-grid'),
    'thumb_url'=>'',
  );
...
function post_grid_get_media($item_post_id, $media_source, $featured_img_size, $thumb_linked){
		...
		if(!empty($post_grid_post_settings[0]['thumb_custom_url'])){
			$thumb_custom_url = $post_grid_post_settings[0]['thumb_custom_url'];
				...
				if(!empty($thumb_url)){
					if($thumb_linked=='yes'){
						...
						if(!empty($thumb_custom_url)){
							...
							$html_thumb.= '<a href="'.get_permalink($item_post_id).'"><img alt="'.$alt_text.'" src="'.$thumb_url.'" /></a>';
							}
		...
		elseif($media_source == 'empty_thumb'){
				...
				if($thumb_linked=='yes'){
					$html_thumb.= '<a class="custom" href="'.get_permalink($item_post_id).'"><img src="'.post_grid_plugin_url.'assets/frontend/css/images/placeholder.png" /></a>';
					}
				else{
		...
		elseif($media_source == 'custom_thumb'){
				...
				if(!empty($custom_thumb_source)){
					...
					if($thumb_linked=='yes'){
						$html_thumb.= '<a href="'.get_permalink($item_post_id).'"><img src="'.$custom_thumb_source.'" /></a>';
						}
					else{
		...
		elseif($media_source == 'font_awesome'){
				...
				if(!empty($custom_thumb_source)){
					...
					if($thumb_linked=='yes'){
						$html_thumb.= '<a href="'.get_permalink($item_post_id).'"><i style="color:'.$font_awesome_icon_color.';font-size:'.$font_awesome_icon_size.'" class="fa '.$font_awesome_icon.'"></i></a>';
						}
					else{
		...
		elseif($media_source == 'first_image'){
			...
			if(empty($first_img)) {
				...
				if($thumb_linked=='yes'){
					$html_thumb.= '<a href="'.get_permalink($item_post_id).'"><img src="'.$first_img.'" /></a>';
					}
				else{
...
function post_grid_meta_query_args($meta_query){
	...
	foreach($meta_query as $key=>$meta_info){
		...
		<div class="items">
            ...
            <span class="expand-collapse pg-tooltip" title="<?php echo __('Expand or collapse', post_grid_textdomain); ?>">
...
function post_grid_get_categories($post_id){
	...
	if(current_user_can('manage_options')){
		...
		if(!empty($taxonomies)){
			...
			echo __('No categories found.', post_grid_textdomain);
			}
...
function post_grid_layout_add_elements(){
	...
	if(current_user_can('manage_options')){
		...
		$item_name = $layout_items[$item_key]['name'];
		$item_html = $layout_items[$item_key]['dummy_html'];
		$item_css = $layout_items[$item_key]['css'];		
		...
		<span class="remove pg-tooltip" title="'.__('Remove', post_grid_textdomain).'"><i class="fa fa-times"></i></span>
		<span class="move pg-tooltip" title="'.__('Move', post_grid_textdomain).'"><i class="fa fa-bars"></i></span>
		<span class="expand pg-tooltip" title="'.__('Expand or collapse', post_grid_textdomain).'">
			<i class="fa fa-expand"></i>
		...
		if($item_key=='meta_key'){
			...
			$html['options'].= ''.__('Meta Key:', post_grid_textdomain).' <br /><input type="text" value="" name="post_grid_layout_content['.$layout.']['.$unique_id.'][field_id]" /><br /><br />';
			$html['options'].= ''.__('Wrapper:', post_grid_textdomain).' <br />use %s where you want to repalce the meta value. Example<pre>&lt;div&gt;%s&lt;/div&gt;</pre> <br /><input type="text" value="%s" name="post_grid_layout_content['.$layout.']['.$unique_id.'][wrapper]" /><br /><br />';		
		...
		if($item_key=='html'){
			...
			$html['options'].= ''.__('Custom HTML:', post_grid_textdomain).' <br /><input type="text" value="" name="post_grid_layout_content['.$layout.']['.$unique_id.'][html]" /><br /><br />';		
		...
		if($item_key=='read_more' || $item_key=='excerpt_read_more'){
			...
			$html['options'].= ''.__('Read more text:', post_grid_textdomain).' <br /><input type="text" value="" name="post_grid_layout_content['.$layout.']['.$unique_id.'][read_more_text]" /><br /><br />';
			}		
			
		if($item_key=='five_star'){
			
			$html['options'].= ''.__('Five star count:', post_grid_textdomain).' <br /><input type="text" value="" name="post_grid_layout_content['.$layout.']['.$unique_id.'][five_star_count]" /><br /><br />';
			}		
			
		if($item_key=='custom_taxonomy'){
			
			$html['options'].= ''.__('Taxonomy:', post_grid_textdomain).' <br /><input type="text" value="" name="post_grid_layout_content['.$layout.']['.$unique_id.'][taxonomy]" /><br /><br />';
			$html['options'].= ''.__('Term count:', post_grid_textdomain).' <br /><input type="text" value="" name="post_grid_layout_content['.$layout.']['.$unique_id.'][taxonomy_term_count]" /><br /><br />';			
			}		
		...
		if($item_key=='up_arrow' || $item_key=='down_arrow' ){
			...
			$html['options'].= ''.__('Arrow size(px):', post_grid_textdomain).' <br /><input type="text" placeholder="10px" value="" name="post_grid_layout_content['.$layout.']['.$unique_id.'][arrow_size]" /><br /><br />';
			$html['options'].= ''.__('Background color:', post_grid_textdomain).' <br /><input class="color" type="text" value="" name="post_grid_layout_content['.$layout.']['.$unique_id.'][arrow_bg_color]" /><br /><br />';		
			}		
		...
		if($item_key=='title'  || $item_key=='title_link'  || $item_key=='excerpt' || $item_key=='excerpt_read_more' ){
			...
			$html['options'].= ''.__('Character limit:', post_grid_textdomain).' <br /><input type="text" value="20" name="post_grid_layout_content['.$layout.']['.$unique_id.'][char_limit]" /><br /><br />';
			}
		...
		if($item_key=='title_link' || $item_key=='read_more' || $item_key=='excerpt_read_more'  ){
			...
			$html['options'].= ''.__('Link target:', post_grid_textdomain).' <br />
		...
		<textarea class="custom_css" item_id="'.$unique_id.'" name="post_grid_layout_content['.$layout.']['.$unique_id.'][css]"  style="width:50%" spellcheck="false" autocapitalize="off" autocorrect="off">'.$item_css.'</textarea><br /><br />
		
		CSS Hover: <br />
		<textarea class="custom_css" item_id="'.$item_key.'" name="post_grid_layout_content['.$layout.']['.$unique_id.'][css_hover]"  style="width:50%" spellcheck="false" autocapitalize="off" autocorrect="off"></textarea>';
...
function post_grid_active_filter(){
	...
	if(current_user_can('manage_options')){
			...
			$html .= '<option  value="all">'.__('All', post_grid_textdomain).'</option>';
